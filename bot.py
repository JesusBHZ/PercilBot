...
def image(bot, update):
    try:
        ...
        res = clasificador.CilantroPerejil(filename)
        update.message.reply_text(f"Es un {res[0]} y estoy {res[1]}% de seguro")
    except Exception as e:
        print(f"Error image: {e.args}")

def main():
    try:
        print("PercilBot iniciando el token")
        updater = Updater(token)

        print("PercilBot iniciando dispacher")
        db = updater.dispatcher

        print("PercilBot iniciando commandHandler")
        ...
        print("PercilBot iniciando el bot")
        updater.idle()
